...
public class Chart<T> extends JQueryGenericContainer<List<T>> implements IChartListener // NOSONAR
{
	...
	{
		this(id, new ListModel<T>(data), series, options, new JsonConverter<T>());
	}
	...
	{
		this(id, new ListModel<T>(data), series, new Options(), converter);
	}
	...
	{
		this(id, new ListModel<T>(data), series, options, converter);
	}
